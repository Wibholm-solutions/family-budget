...
class TestValidationResult:
    ...
    def test_not_ok_with_errors(self):
        result = ValidationResult(errors=["bad"], parsed={})
        assert result.ok is False

    def test_raise_if_invalid_raises_400(self):
        result = ValidationResult(errors=["error one", "error two"], parsed={})
        with pytest.raises(HTTPException) as exc_info:
            result.raise_if_invalid()
        assert exc_info.value.status_code == 400
        assert "error one" in exc_info.value.detail
        assert "error two" in exc_info.value.detail
...
class TestValidateExpense:
    ...
    def test_semi_annual_with_months(self):
        result = validate_expense("Forsikring", "6000", "semi-annual", "1,7", None)
        assert result.ok
