...
def _validate_name(name: str | None, entity: str, errors: list[str]) -> str | None:
    if name is None or not name.strip():
        errors.append(f"{entity}-navn må ikke være tomt")
        return None
    stripped = name.strip()
    if len(stripped) > 200:
...
def validate_expense(
    ...
    parsed["name"] = _validate_name(name, "Udgift", errors)
    parsed["amount"] = _parse_and_validate_amount(amount_str, errors)
    freq = _validate_frequency(frequency, errors)
    parsed["frequency"] = freq
    parsed["months"] = _parse_and_validate_months(
        months_str if months_str else None, frequency, errors
    ) if freq else None
